...
function removeParamURL($url)
{
    ...
    if ($getQueryP) $url = substr($url, 0, $getQueryP); //  remove params
    return $url;
}
...
function dispathRoute($url)
{
    ...
    $url = trim($url, '/');
    $url = removeParamURL($url);
    if (isset($routes[$url])) call_user_func($routes[$url]);
    else
    {
        global $blade;
        $PathPageNotFound = '404Page';
        echo $blade->run($PathPageNotFound, ['title' => "Opps! Not Found"]);
    }
}

...
function mergeResult($arrOrigin, $arrMerge, $keyToValidate, $data, $keyOfElement = false)
{
    ...
    foreach ($data as $e)
    {
        ...
        if (!isset($arrResult[$idProduct]))
        { //  if empty arrResult don't have current id product
            for ($i = 0;$i < count($arrOrigin);$i++)
            {
                ...
                if ($keyOfElement && array_key_exists($nameKeyOrigin, $keyOfElement))
                {
                    ...
                    $e[$nameKeyMerge] = [];
                    else $e[$nameKeyMerge][$keyMergeElement] = $valueOfKeyOrigin;
                }
                else
                {
                    if (empty($valueOfKeyOrigin)) //  Check if value of key origin is empty
                    $e[$nameKeyMerge] = [];
                    else $e[$nameKeyMerge] = [$valueOfKeyOrigin]; //  create empty Category list and add first value
        ...
        { //  Exist
            for ($i = 0;$i < count($arrOrigin);$i++)
            {
                ...
                $nameKeyMerge = $arrMerge[$i];
                $valueOfKeyOrigin = $e[$nameKeyOrigin]; //  get KEY origin of duplicate product, eg: $e['name']
                if (!in_array($valueOfKeyOrigin, $arrResult[$idProduct][$nameKeyMerge]) && !empty($valueOfKeyOrigin))
                { //  Check if have already
                    ...
                    else array_push($arrResult[$idProduct][$nameKeyMerge], $valueOfKeyOrigin); // If not push to list category
...
function checkEmpty($arr)
{
    foreach ($arr as $val)
    {
        if (empty($val) && $val !== 0) return true;
    }
    return false;
}
...
function createCheckQuery($arr)
{
    ...
    {
        if (!is_numeric($val)) $val = addApostrophe($val);
        $values[] = $field . "=" . $val;
    }
...
function checkEmptyFile($file, $type)
{
    ...
    if ($type === 2)
    { //  multipe
        if (empty($file['size'][0])) return true;
        else return false;
    }

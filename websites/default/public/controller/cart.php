...
class CartController extends Controller
{
    ...
    function __construct()
    {
        ...
        $this->fileRender = ['checkout' => 'cart.checkout', 'cart' => 'cart.wishlist'];
        $this->apiContext = new ApiContext(new OAuthTokenCredential(CLIENT_ID, // ClientID
        CLIENT_SECRET
        // ClientSecret
        ));
    }

    public function itemInCart($cart, $idItem)
    ...
    public function getAdditionInfoProd($listItem)
    { //  Get addition field for product(id)
        //  Set up condition
        $condition = ['products.title' => []];
        ...
        $info = $this
            ->prodModel
            ->selectMany(['id', 'title'], null, $condition);
        //  Loop through list item
        foreach ($listItem as $key => $item)
        {
            ...
            { //  Loop through addition info result
                ...
                if ($titleProd == $element['title']) $listItem[$key] = array_merge_recursive($listItem[$key], $element);
            }
    ...
    public function action()
    {
        ...
        $getCategoryHeader = $this
            ->model
            ->getAllCategory();
        $this->render($this->fileRender['cart'], ['title' => 'cart', 'carts' => $cart, 'categoryHeader' => $getCategoryHeader]);
    }
    ...
    { //  Add new Product to cart
        ...
        $prod = $this
            ->prodModel
            ->getProductWithId($_POST['id'], ['products.price', 'products.title', 'products.image']);
        ...
        //  Create new item
        $item = ['product_id' => $_POST['id'], 'product_image' => $prod[0]['image'], 'quantity' => $_POST['quantity'], 'title' => $prod[0]['title'], 'price' => $prod[0]['price'], 'priceTotal' => (double)$prod[0]['price'] * (int)$_POST['quantity']];
    ...
    { //  Remove product
        ...
        if ($keyItem == NULL) $keyItem = $this->itemInCart($_SESSION['cart'], $_POST['id']);
        if ($keyItem === false)
        { // If not found
    ...
    public function checkout()
    {
        ...
        $getCategoryHeader = $this
            ->model
            ->getAllCategory();
        $this->render($this->fileRender['checkout'], ['title' => 'Checkout', 'carts' => $cart, 'categoryHeader' => $getCategoryHeader]);
        return;
    }

    public function postCheckout()
    { //  Request for  make charge
    ...
    { //  Set up and create payment then generate link for user payment
        //  Check empty field
        ...
        $addressPayer->setLine1($_POST['address'])->setPhone($_POST['phone'])->setCity('Singapore')
            ->setCountryCode('SG')
            ->setPostalCode('02')
            ->setLine2($_POST['address'])->setPhone($_POST['phone']);

        //  Set payer info
        $payerInf = new PayerInfo();
        $payerInf->setEmail($_POST['email'])->setFirstName($_SESSION['user'])->setBillingAddress($addressPayer);
        ...
        {
            $item1 = new Item();
            $item1->setName($item['title'])->setCurrency('USD')
                ->setQuantity($item['quantity'])->setPrice($item['price']);
            $listItem[] = $item1;
        }
        ...
        //  Amount to charge
        $amount = new Amount();
        $amount->setTotal($_SESSION['cart']['totalPrice'])->setCurrency('USD')
            ->setDetails($detail);
        ...
        $payment->setIntent('sale')
            ->setPayer($payer)->setTransactions(array(
            $transaction
        ))->setRedirectUrls($redirectUrls);

        // Notice from user
        if (!empty($_POST['notice'])) $payment->setNoteToPayer($_POST['notice']);
    ...
    { //  Checkout success, create Order save to DB
        if (empty($_SESSION['user']))
        {
        ...
        $checkOrderExist = $this
            ->prodModel
            ->select(NULL, ['paymentID' => $paymentId], 'orders');
        if ($checkOrderExist)
        { //  If exist
        ...
        try
        {
            ...
            $userID = $this
                ->userModel
                ->select(['user_id'], ['user_id' => $_SESSION['user']]);
            if (!$userID)
            { //   If not have
    ...
    { //  Create new Order to DB
        ...
        if (!isset($payment['transactions'][0]['item_list']['shipping_address'], $payment['payer']['payer_info']['email'], $payment['state'], $payment['id'])) return false;
    ...
    { // Create cart and cart_item to DB
        if (empty($cart) || empty($orderID) || empty($userID)) return false;
        ...
        $this
            ->prodModel
            ->insert($cartItemVal, NULL, 'cart_item');
        return true;
    }
}
